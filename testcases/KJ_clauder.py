    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            ...
            if test_case.start_city == test_case.end_city:
                ...
                route1 = planner.least_flights_earliest_route(
            ...
            route1 = Route(tuple(planner.least_flights_earliest_route(
                test_case.start_city, test_case.end_city,
                test_case.time_start, test_case.time_end
            ) or []))
            ...
            expected_route1 = min((r for r in all_routes if r.num_flights == min(r.num_flights for r in all_routes)), key=lambda r: r.arrival_time)
...
if __name__ == "__main__":
    ...
    all_test_cases = [
        ...
        TestCase("Multiple Routes Same Cost", "Multiple routes with same total fare but different flight counts.", 0, 4, 0, 700),
        ...
        if route_name == "Route 1 (Least Flights, Earliest)":
            ...
            if current_route_fare > min_fare:
                ...
                for valid_route in all_valid_routes:
                    ...
                    print(f"Route: {[f'{f.start_city}->{f.end_city} (Fare: {f.fare})' for f in valid_route]}, Total Fare: {valid_route_fare}")
